...
# Luarmor
LUARMOR_API_KEY = os.environ.get("LUARMOR_API_KEY", "").strip()
LUARMOR_BASE = "https://api.luarmor.net"
...
async def on_ready():
    ...
    # Register persistent Luarmor panel view (buttons survive restarts)
    try:
        client.add_view(LuarmorPanel())
    except Exception as e:
...
# ============= LUARMOR =============
LUARMOR_LOADER_URL_TMPL = "https://api.luarmor.net/files/v3/loaders/{script_id}.lua"


class Luarmor:
    """Minimal async client for the Luarmor v3 API."""
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.base = LUARMOR_BASE
        self._headers = {
    ...
    async def _req(self, method: str, path: str, **kwargs):
        ...
        async with httpx.AsyncClient(timeout=15, headers=self._headers) as h:
            r = await h.request(method, url, **kwargs)
            try:
                data = r.json()
            except Exception:
                data = {"raw": r.text}
    ...
    async def whitelist(self, project_id: str, discord_id: Optional[str] = None,
                        note: Optional[str] = None) -> dict:
        body = {"user_wanted": "false"}
        if discord_id:
            body["discord_id"] = str(discord_id)
        if note:
            body["note"] = note
        return await self._req("POST", f"/v3/projects/{project_id}/users", json=body)

    async def blacklist(self, project_id: str, user_key: str) -> dict:
        return await self._req(
            "POST", f"/v3/projects/{project_id}/users/blacklist",
            json={"user_key": user_key},
        )

    async def reset_hwid(self, project_id: str, user_key: str, force: bool = False) -> dict:
        return await self._req(
            "POST", f"/v3/projects/{project_id}/users/resethwid",
            json={"user_key": user_key, "force": force},
        )

    async def key_details(self, user_key: str) -> dict:
        return await self._req("GET", f"/v3/keys/{user_key}/details")

    async def find_by_discord(self, project_id: str, discord_id: str) -> dict:
        return await self._req(
            "GET", f"/v3/projects/{project_id}/users?discord_id={discord_id}"
        )


def _lm() -> Optional[Luarmor]:
    if not LUARMOR_API_KEY:
        return None
    return Luarmor(LUARMOR_API_KEY)


def _lm_success(resp: dict) -> bool:
...
class RedeemKeyModal(discord.ui.Modal, title="Redeem Your Key"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        lm = _lm()
        if not lm:
            await interaction.response.send_message(_err("Luarmor is not configured."), ephemeral=True)
            return
        key = str(self.key_input.value).strip()
        # Attach discord id
        r = await lm._req(
            "POST", f"/v3/projects/{self.panel_cfg['project_id']}/users",
            json={"user_wanted": "true", "user_key": key,
        ...
        if _lm_success(r):
            ...
            if role_id and interaction.guild:
                ...
                if role and isinstance(interaction.user, discord.Member):
                    try:
                        await interaction.user.add_roles(role, reason="Luarmor: key redeemed")
                    except Exception:
                        pass
            await interaction.response.send_message(
                _ok("Key linked to your Discord account. Enjoy!"), ephemeral=True)
        else:
            msg = r.get("message") or r.get("raw") or "Unknown error"
            await interaction.response.send_message(_err(f"Redeem failed: {msg}"), ephemeral=True)
...
class ResetHwidModal(discord.ui.Modal, title="Reset HWID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not lm:
            await interaction.response.send_message(_err("Luarmor is not configured."), ephemeral=True)
            return
        r = await lm.reset_hwid(self.panel_cfg["project_id"], str(self.key_input.value).strip())
        if _lm_success(r):
            await interaction.response.send_message(_ok("HWID reset. Re-run the script."),
                                                    ephemeral=True)
        else:
            await interaction.response.send_message(
                _err(f"Reset failed: {r.get('message') or r.get('raw', 'unknown')}"),
                ephemeral=True)


class LuarmorPanel(discord.ui.View):
    """Persistent view. Loads config from Mongo using message_id (custom_id encodes msg id)."""
    def __init__(self):
    ...
    async def _cfg(self, interaction: discord.Interaction) -> Optional[dict]:
        ...
        if not msg_id:
            return None
        cfg = await db.luarmor_panels.find_one({"message_id": str(msg_id)}, {"_id": 0})
        if not cfg:
            await interaction.response.send_message(
                _err("Panel config missing. Ask an admin to run /lm-setpanel again."),
                ephemeral=True)
        return cfg

    @discord.ui.button(label="Redeem Key", style=discord.ButtonStyle.primary,
                       emoji="🔑", custom_id="lm_redeem")
    async def redeem(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await self._cfg(interaction)
        if not cfg: return
        await interaction.response.send_modal(RedeemKeyModal(cfg))

    @discord.ui.button(label="Get Script", style=discord.ButtonStyle.primary,
                       emoji="📥", custom_id="lm_getscript")
    async def getscript(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await self._cfg(interaction)
        if not cfg: return
        loader = LUARMOR_LOADER_URL_TMPL.format(script_id=cfg['script_id'])
        code = f"loadstring(game:HttpGet(\"{loader}\"))()"
        e = discord.Embed(
            title="📥  Your Script",
            description=f"Copy the loader below and paste into your executor:\n```lua\n{code}\n```",
            color=0x007AFF,
        )
        await interaction.response.send_message(embed=e, ephemeral=True)

    @discord.ui.button(label="Get Role", style=discord.ButtonStyle.success,
                       emoji="👤", custom_id="lm_getrole")
    async def getrole(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await self._cfg(interaction)
        if not cfg: return
        lm = _lm()
        if not lm:
            await interaction.response.send_message(_err("Luarmor not configured."),
                                                    ephemeral=True)
            return
        r = await lm.find_by_discord(cfg["project_id"], str(interaction.user.id))
        users = r.get("users") or []
        if not users:
            await interaction.response.send_message(
                _err("No key linked to your Discord account. Redeem a key first."),
                ephemeral=True)
            return
        ...
        if not role:
            ...
            return
        try:
            await interaction.user.add_roles(role, reason="Luarmor: role claim")
            await interaction.response.send_message(_ok(f"Granted {role.mention}"), ephemeral=True)
        except Exception as e:
    ...
    async def getstats(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        e.add_field(name="Key", value=f"||`{u.get('user_key', '?')}`||", inline=False)
        e.add_field(name="Status", value=str(u.get('status', '—')))
        e.add_field(name="Auth Expire", value=str(u.get('auth_expire', '—')))
        e.add_field(name="Total Executions", value=str(u.get('total_executions', '—')))
        e.add_field(name="HWID", value=str(u.get('identifier', '—') or '—'))
        e.add_field(name="Note", value=str(u.get('note', '—') or '—'))
        await interaction.response.send_message(embed=e, ephemeral=True)


# ===== Luarmor slash commands =====
@tree.command(name="lm-setpanel",
              description="Create a Luarmor script panel (Redeem Key / Get Script / Reset HWID …).")
@app_commands.describe(
    script_name="Display name (e.g. 'Custom Loader')",
    project_id="Your Luarmor project ID",
    script_id="Your Luarmor script/loader ID",
    customer_role="Role granted after redeeming a key",
    manager_role="Role required to manage — only members with this role can run /lm-* commands",
)
@guild_only()
@needs_auth()
async def lm_setpanel(
    ...
    if not LUARMOR_API_KEY:
        await _reply(interaction, _err(
            "LUARMOR_API_KEY is not set. Add it in the dashboard Config, or as an env variable."))
        return
    ...
    embed.set_footer(text="Powered by Luarmor · MOD_CTRL")

    await interaction.response.send_message(embed=embed, view=LuarmorPanel())
    msg = await interaction.original_response()

    # Persist panel config so buttons work across restarts
    await db.luarmor_panels.update_one(
        ...
        {"$set": {
            ...
            "manager_role_id": str(manager_role.id) if manager_role else None,
            "created_at": datetime.now(timezone.utc).isoformat(),
        }},
        upsert=True,
    )
    _log(f"[luarmor] panel created: guild={interaction.guild.id} script={script_id}")


@tree.command(name="lm-whitelist", description="Whitelist a Discord user in Luarmor.")
@app_commands.describe(user="Discord user to whitelist", project_id="Luarmor project ID",
                       note="Optional note")
@guild_only()
@needs_auth()
async def lm_whitelist(interaction: discord.Interaction, user: discord.Member,
                       project_id: str, note: Optional[str] = None):
    lm = _lm()
    if not lm:
        await _reply(interaction, _err("LUARMOR_API_KEY is not set."))
        return
    await interaction.response.defer(ephemeral=True)
    r = await lm.whitelist(project_id, discord_id=str(user.id), note=note)
    if _lm_success(r):
        key = r.get("user_key") or r.get("key") or "—"
        try:
            await user.send(
                f"🔑 You've been whitelisted!\nYour key: ||`{key}`||\n"
                f"Keep it private."
            )
            dm_note = "DM sent."
        except Exception:
            dm_note = "Couldn't DM the user (their DMs are closed)."
        await interaction.followup.send(
            _ok(f"Whitelisted {user.mention}. Key generated. {dm_note}"), ephemeral=True)
    else:
        await interaction.followup.send(
            _err(f"Failed: {r.get('message') or r.get('raw', 'unknown')}"), ephemeral=True)


@tree.command(name="lm-blacklist", description="Blacklist a Luarmor key.")
@guild_only()
@needs_auth()
async def lm_blacklist(interaction: discord.Interaction, project_id: str, user_key: str):
    lm = _lm()
    if not lm:
        await _reply(interaction, _err("LUARMOR_API_KEY is not set."))
        return
    r = await lm.blacklist(project_id, user_key)
    if _lm_success(r):
        await _reply(interaction, _ok(
            f"Key blacklisted. Unban token: ||`{r.get('unban_token', '—')}`||"))
    else:
        await _reply(interaction, _err(
            f"Failed: {r.get('message') or r.get('raw', 'unknown')}"))


@tree.command(name="lm-resethwid", description="Force-reset HWID for a Luarmor key.")
@guild_only()
@needs_auth()
async def lm_resethwid(interaction: discord.Interaction, project_id: str, user_key: str):
    lm = _lm()
    if not lm:
        await _reply(interaction, _err("LUARMOR_API_KEY is not set."))
        return
    r = await lm.reset_hwid(project_id, user_key, force=True)
    if _lm_success(r):
...
@tree.command(name="lm-keyinfo", description="Look up details of a Luarmor key.")
@guild_only()
@needs_auth()
async def lm_keyinfo(interaction: discord.Interaction, user_key: str):
    lm = _lm()
    if not lm:
        await _reply(interaction, _err("LUARMOR_API_KEY is not set."))
        return
    r = await lm.key_details(user_key)
    if not _lm_success(r):
        await _reply(interaction, _err(
            f"Failed: {r.get('message') or r.get('raw', 'unknown')}"))
        return
    e = discord.Embed(title="🔑 Key Details", color=0x007AFF)
    e.add_field(name="Status", value=str(r.get("status", "—")))
    e.add_field(name="Discord ID", value=str(r.get("discord_id", "—")))
    e.add_field(name="HWID", value=str(r.get("identifier", "—") or "—"))
    e.add_field(name="Executions", value=str(r.get("total_executions", "—")))
    e.add_field(name="Expires", value=str(r.get("auth_expire", "—")))
    e.add_field(name="Note", value=str(r.get("note", "—") or "—"))
    await _reply(interaction, "", embed=e, ephemeral=False)
